...
func loadTranslations(lang string) {
	...
	if err != nil {
		logger.Fatalf("Error loading localization (%s): %v", lang, err)
	}

	defer func(file *os.File) {
		err := file.Close()
		if err != nil {
			logger.Fatalf("Error closing localization (%s): %v", lang, err)
		}
	}(file)
	...
	if err := decoder.Decode(&translations); err != nil {
		logger.Fatalf("JSON parsing error (%s): %v", lang, err)
	}
	logger.Infof("Localization loaded: %s", lang)
}
